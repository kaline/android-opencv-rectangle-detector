...
public class MainActivity extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
        ...
        public void onManagerConnected(int status) {
            switch(status){
                case BaseLoaderCallback.SUCCESS:{
                    ...
                    Log.v("kaline-log", "Loader interface success");
                    bwIMG = new Mat();
                    dsIMG = new Mat();
                    hsvIMG = new Mat();
                    lrrIMG = new Mat();
    ...
    protected void onResume(){
        ...
        if(OpenCVLoader.initDebug()){
            Log.i(TAG,"OPENCV LOADED FINE");
            mLoaderCallBack.onManagerConnected(LoaderCallbackInterface.SUCCESS);

        }else{
            Log.i(TAG, "OPENCB NOT LOADED");
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        for(MatOfPoint cnt : contours){
            ...
            if(numberVertices >= 4 && numberVertices <= 6 ){
                ...
                for(int j = 2; j < numberVertices; j++) {
    ...
    private static Double angle(Point point1, Point point2, Point point0) {
        double dx1 = point1.x - point0.x;
        double dy1 = point1.y - point0.y;
        double dx2 = point2.x = point0.x;
        double dy2 = point2.y - point0.y;

        return (dx1 * dx2 + dy1 * dy2)/Math.sqrt((dx1 * dx1 + dy1 *dy1) *(dy1 * dy1 + dy2 *dy2) +1e-10);
